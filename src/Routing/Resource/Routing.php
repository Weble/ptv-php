...
class Routing extends Resource
{
     ...
     * @return $this
     */
    public function with(array|ResultType $results): self
    {
        $this->results = $results instanceof ResultType ? [$results] : $results;
        return $this;
    }

    public function withResultType(ResultType $type): self
    {
    ...
    /**
     * @param array $waypoints The list of waypoints the route will be calculated for. At least two waypoints are necessary, a maximum number may apply according to your subscription.
    ...
    ): Route
    {
        ...
        if (is_array($results)) {
            $results = array_map(fn(ResultType|string $resultType): string => $resultType instanceof ResultType ? $resultType->value : $resultType, $results);
        }
     ...
     */
    public function getRouteByRouteId(string $routeId): Response
    {
        return $this->connector->send(new GetRouteByRouteId($routeId));
    }
