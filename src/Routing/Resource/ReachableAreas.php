...
use PTV\Routing\Requests\ReachableAreas\CalculateReachableAreas;
use PTV\Routing\Requests\ReachableAreas\DeleteReachableAreas;
use PTV\Routing\Requests\ReachableAreas\GetReachableAreas;
use PTV\Routing\Requests\ReachableAreas\StartAndCreateReachableAreas;
use PTV\Routing\Resource;
use Saloon\Http\Response;

class ReachableAreas extends Resource
{
	/**
	 ...
	 */
	public function calculateReachableAreas(
		string $waypoint,
		?string $profile,
		array $horizons,
		?string $horizonType,
	...
	{
		return $this->connector->send(new CalculateReachableAreas($waypoint, $profile, $horizons, $horizonType, $options));
	}
	 ...
	 * Use array notation like `options[trafficMode]=AVERAGE` to set options.
	 */
	public function startAndCreateReachableAreas(
		?string $waypoint,
		?string $routeId,
		?string $profile,
		array $horizons,
	...
	{
		return $this->connector->send(new StartAndCreateReachableAreas($waypoint, $routeId, $profile, $horizons, $horizonType, $options));
	}
	 ...
	 */
	public function getReachableAreas(string $id): Response
	{
		return $this->connector->send(new GetReachableAreas($id));
	}
	 ...
	 * @param string $id The ID of the calculated reachable areas.
	 */
	public function deleteReachableAreas(string $id): Response
	{
		return $this->connector->send(new DeleteReachableAreas($id));
	}
}
